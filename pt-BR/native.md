# Bindings Nativos & Extensibilidade em Aly

Aly suporta extensibilidade em tempo de execução e bindings nativos de módulos Rust através de traits de registro nativos e bibliotecas dinâmicas.

---

## 1. Integração com Rust

Para vincular métodos Rust como funções nativas dentro do Aly, você mapeia tipos Rust usando a representação `ValueData` em módulos Rust (como `src/native/`):
...
Plugins de bibliotecas externas (`.so` / `.dylib` / `.dll`) podem ser carregados dinamicamente no interpretador usando:

```aly
plugin import "caminho/para/plugin"
```

Isso registra símbolos, métodos e estruturas externos no namespace ativo de execução do Aly.

---

## 3. Importando um plugin
...
O caminho é relativo ao diretório atual de execução. O `as nome` define o namespace usado para chamar as funções do plugin. Se omitido, o nome do arquivo (sem extensão) é usado como namespace.
...
Todo plugin deve exportar três funções obrigatórias descritas no header `plugin_sdk/plugin.h`:
...
Retorna um ponteiro para uma struct `AlyPluginInfo` estática. Chamada uma única vez ao carregar o plugin. Nunca deve retornar NULL.
...
Receibe o nome da função e uma string de argumentos. Deve retornar uma string alocada com `malloc` (ou equivalente) que será liberada via `aly_plugin_free_string`. Retorne NULL para indicar erro.
...
Libera a string retornada por `aly_plugin_call`. Se ptr for NULL, a chamada é ignorada.
...
Retorna uma string com os nomes das funções separados por vírgula. Se exportada, cada função é registrada individualmente como `namespace.nomefuncao`, permitindo chamadas diretas como `math.add(3, 7)`. Se não exportada, o runtime registra apenas o namespace sem funções individuais.
...
## 11. Como o Aly carrega plugins

Internally, o Aly usa `libloading` (Rust) para carregar a biblioteca compartilhada em tempo de execução via `dlopen`/`dlsym` (Linux), `LoadLibrary`/`GetProcAddress` (Windows) ou `dlopen`/`dlsym` (macOS).
...
5. Quando `nome.funcao(args)` é chamado, o Aly converte os argumentos para string e chama `aly_plugin_call("funcao", "arg1,arg2,...")`
...
Plugins são código nativo executado no mesmo processo do Aly. Eles têm acesso total ao sistema (arquivos, rede, etc). Carregue apenas plugins de fontes confiáveis.
