mod crypto_mod {
    ...
    // crypto.argon2_hash(password) -> string (argon2id encoded hash)
    pub fn crypto_argon2_hash(x: String) -> Box<dyn Validator> {
        use argon2::{Argon2, PasswordHasher};
        use argon2::password_hash::{SaltString, rand_core::OsRng};
        let args = split_args(&x, 1);
        let password = std_arg(&args, 0);

        let salt = SaltString::generate(&mut OsRng);
        let argon2 = Argon2::default();
        match argon2.hash_password(password.as_bytes(), &salt) {
            Ok(hash) => Box::new(put_quoted_str(hash.to_string())),
            Err(e) => {
                eprintln!("RuntimeError [crypto.argon2_hash]: erro ao gerar hash: {}", e);
                Box::new(put_quoted_str(String::new()))
            }
        }
    }

    // crypto.argon2_verify(hash, password) -> bool
    pub fn crypto_argon2_verify(x: String) -> Box<dyn Validator> {
        use argon2::{Argon2, PasswordVerifier};
        use argon2::password_hash::PasswordHash;
        let args = split_args(&x, 2);
        let hash = std_arg(&args, 0);
        let password = std_arg(&args, 1);

        let parsed_hash = match PasswordHash::new(&hash) {
            Ok(h) => h,
            Err(e) => {
                eprintln!("RuntimeError [crypto.argon2_verify]: hash inválido: {}", e);
                return Box::new(ValueData::Bool(false));
            }
        };

        let argon2 = Argon2::default();
        Box::new(ValueData::Bool(argon2.verify_password(password.as_bytes(), &parsed_hash).is_ok()))
    }

    // crypto.bcrypt_hash(password) -> string (bcrypt encoded hash)
    pub fn crypto_bcrypt_hash(x: String) -> Box<dyn Validator> {
        let args = split_args(&x, 1);
        let password = std_arg(&args, 0);

        match bcrypt::hash(&password, bcrypt::DEFAULT_COST) {
            Ok(hash) => Box::new(put_quoted_str(hash)),
            Err(e) => {
                eprintln!("RuntimeError [crypto.bcrypt_hash]: erro ao gerar hash: {}", e);
                Box::new(put_quoted_str(String::new()))
            }
        }
    }

    // crypto.bcrypt_verify(hash, password) -> bool
    pub fn crypto_bcrypt_verify(x: String) -> Box<dyn Validator> {
        let args = split_args(&x, 2);
        let hash = std_arg(&args, 0);
        let password = std_arg(&args, 1);

        match bcrypt::verify(&password, &hash) {
            Ok(valid) => Box::new(ValueData::Bool(valid)),
            Err(e) => {
                eprintln!("RuntimeError [crypto.bcrypt_verify]: erro ao verificar hash: {}", e);
                Box::new(ValueData::Bool(false))
            }
        }
    }
