mod tuple {
    ...
    pub struct Tuple(Vec<ValueData>);

    impl Tuple {
        pub fn new(value: Vec<ValueData>) -> Tuple {
        ...
        pub fn to_string(&self, json: bool) -> String {
            let mut string = String::from("Tuple (#data");
    ...
    pub fn create_tuple(lexer: Vec<Lexer>) -> Box<dyn Validator>{
        let mut another = 0;
        let mut new_vec = vec![];
        let mut values = vec![];
        ...
        let tup = Tuple::new(values);

        return Box::new(ValueData::Tuple(tup));
    }
}
