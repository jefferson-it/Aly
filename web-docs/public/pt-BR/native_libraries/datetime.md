...
| `datetime_parse(date_str, format)` | `date_str: string`, `format: string` | `string` | Analisa uma `date_str` usando um `format` personalizado (ex: `"%d/%m/%Y"`). Retorna uma string ISO-8601. |
| `datetime_from_timestamp(ts_str)` | `ts_str: string` | `string` | Converte um timestamp Unix (em segundos) em uma string de data e hora ISO-8601. |
...
- As funções de data e hora retornam valores envoltos em strings para serialização e transferência seguras.
- A análise e a formatação de fuso horário são tratadas internamente usando a crate Rust `chrono`, correspondendo ao fuso horário local do sistema.
