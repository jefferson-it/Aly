...
typedef struct aly_object_entry aly_object_entry_t;
...
aly_value_t aly_int(long long v) { aly_value_t r; r.type = ALY_INT; r.as.int_val = v; r.type_tag = NULL; r.is_mutable = 1; return r; }
aly_value_t aly_float(double v) { aly_value_t r; r.type = ALY_FLOAT; r.as.float_val = v; r.type_tag = NULL; r.is_mutable = 1; return r; }
aly_value_t aly_bool(int v) { aly_value_t r; r.type = ALY_BOOL; r.as.bool_val = v; r.type_tag = NULL; r.is_mutable = 1; return r; }
aly_value_t aly_none(void) { aly_value_t r; r.type = ALY_NONE; r.as.int_val = 0; r.type_tag = NULL; r.is_mutable = 1; return r; }
aly_value_t aly_function(void* f) { aly_value_t r; r.type = ALY_FUNCTION; r.as.func_val = f; r.type_tag = NULL; r.is_mutable = 1; return r; }
...
aly_value_t aly_ref_val(void* ptr) {
    aly_value_t r;
    r.type = ALY_REF;
    r.as.ref_val = ptr;
    r.type_tag = NULL;
...
static inline aly_value_t aly_clone(aly_value_t v) {
    switch (v.type) {
...
const char* aly_type_name(aly_value_t v) {
    switch (v.type) {
        ...
        case ALY_REF: return "reference";
        default: return "unknown";
    }
}

static inline aly_value_t aly_to_int(aly_value_t v) {
    switch (v.type) {
...
aly_value_t aly_band(aly_value_t a, aly_value_t b) {
    return aly_int(aly_to_int(a).as.int_val & aly_to_int(b).as.int_val);
}
aly_value_t aly_bor(aly_value_t a, aly_value_t b) {
    return aly_int(aly_to_int(a).as.int_val | aly_to_int(b).as.int_val);
}
aly_value_t aly_bxor(aly_value_t a, aly_value_t b) {
    return aly_int(aly_to_int(a).as.int_val ^ aly_to_int(b).as.int_val);
}
aly_value_t aly_bnot(aly_value_t a) {
    return aly_int(~aly_to_int(a).as.int_val);
}
aly_value_t aly_shl(aly_value_t a, aly_value_t b) {
    return aly_int(aly_to_int(a).as.int_val << aly_to_int(b).as.int_val);
}
aly_value_t aly_shr(aly_value_t a, aly_value_t b) {
    return aly_int(aly_to_int(a).as.int_val >> aly_to_int(b).as.int_val);
}
...
aly_value_t aly_div(aly_value_t a, aly_value_t b) {
    if (a.type == ALY_INT && b.type == ALY_INT) {
        if (b.as.int_val == 0) { fprintf(stderr, "Error: Division by zero\n"); exit(1); }
...
aly_value_t aly_tomb(aly_value_t v) {
    if (v.type == ALY_REF && v.as.ref_val) {
        ((aly_value_t*)v.as.ref_val)->is_mutable = 0;
    }
    return v;
}

aly_array_t* aly_array_new(long long capacity) {
    aly_array_t* arr = (aly_array_t*)calloc(1, sizeof(aly_array_t));
    arr->capacity = (int)(capacity > 0 ? capacity : 8);
...
static inline char* bool_arr_new(long long n) { return (char*)calloc((size_t)n, 1); }
static inline int bool_arr_get(char* a, long long i) { return a[i]; }
static inline void bool_arr_set(char* a, long long i, int v) { a[i] = (char)v; }

static inline aly_value_t aly_array_get(aly_value_t arr, aly_value_t idx) {
...
static inline aly_value_t aly_array_get_i(aly_value_t arr, long long i) {
    if (arr.type != ALY_ARRAY || !arr.as.array_val) return aly_none();
    if (i < 0 || i >= arr.as.array_val->size) return aly_none();
    return aly_clone(arr.as.array_val->items[(int)i]);
}

static inline void aly_array_set(aly_value_t arr, aly_value_t idx, aly_value_t val) {
    ...
    a->items[i] = aly_clone(val);
}

static inline void aly_array_set_i(aly_value_t arr, long long i, aly_value_t val) {
    if (arr.type != ALY_ARRAY || !arr.as.array_val) return;
    if (i < 0) return;
    aly_array_t* a = arr.as.array_val;
    if (i >= a->capacity) {
    ...
    if (i >= a->size) a->size = (int)i + 1;
    a->items[(int)i] = aly_clone(val);
}
...
void aly_object_set(aly_value_t obj, const char* key, aly_value_t value) {
    ...
    for (int i = 0; i < o->capacity; i++) {
        ...
        if (o->entries[probe].used && strcmp(o->entries[probe].key, key) == 0) {
            if (!o->entries[probe].value.is_mutable) {
                fprintf(stderr, "TypeError: A variável '%s' é constante/imutável, não é possível alterar seu valor.\n", key);
                exit(1);
            }
...
aly_value_t* aly_object_get_ptr(aly_value_t obj, const char* key) {
    if (obj.type != ALY_OBJECT || !obj.as.object_val) return NULL;
    aly_object_t* o = obj.as.object_val;
    unsigned int idx = aly_hash(key) % o->capacity;
    ...
    for (int i = 0; i < o->capacity; i++) {
        ...
        if (!o->entries[probe].used) {
            o->entries[probe].key = strdup(key);
            o->entries[probe].value = aly_none();
            o->entries[probe].used = 1;
            o->size++;
            return &o->entries[probe].value;
        }
...
aly_value_t aly_simd_op(aly_value_t v1, aly_value_t v2, const char* op) {
    if (v1.type != ALY_ARRAY || !v1.as.array_val || v2.type != ALY_ARRAY || !v2.as.array_val) {
        return aly_none();
    }
    int len = v1.as.array_val->size < v2.as.array_val->size ? v1.as.array_val->size : v2.as.array_val->size;
    aly_array_t* arr = aly_array_new(len);
    if (len <= 0) {
        aly_value_t r;
        r.type = ALY_ARRAY;
        r.as.array_val = arr;
    ...
    double* a_floats = (double*)malloc(len * sizeof(double));
    double* b_floats = (double*)malloc(len * sizeof(double));
    double* r_floats = (double*)malloc(len * sizeof(double));
    for (int i = 0; i < len; i++) {
        aly_value_t x = v1.as.array_val->items[i];
        a_floats[i] = (x.type == ALY_INT) ? (double)x.as.int_val : ((x.type == ALY_FLOAT) ? x.as.float_val : 0.0);
        aly_value_t y = v2.as.array_val->items[i];
        b_floats[i] = (y.type == ALY_INT) ? (double)y.as.int_val : ((y.type == ALY_FLOAT) ? y.as.float_val : 0.0);
    }
    if (op[0] == '+') {
        #pragma GCC ivdep
        for (int i = 0; i < len; i++) {
            r_floats[i] = a_floats[i] + b_floats[i];
        }
    } else if (op[0] == '-') {
        #pragma GCC ivdep
        for (int i = 0; i < len; i++) {
        ...
        #pragma GCC ivdep
        for (int i = 0; i < len; i++) {
            r_floats[i] = b_floats[i] != 0.0 ? a_floats[i] / b_floats[i] : 0.0;
        }
    }
    for (int i = 0; i < len; i++) {
        arr->items[i] = aly_float(r_floats[i]);
        arr->size++;
    }
    free(a_floats);
    free(b_floats);
