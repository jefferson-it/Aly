    ...
    pub fn exec(lexers: &mut Vec<Lexer>, val: &mut Box<dyn Validator>) {
        ...
        match to_made {
            ...
            "math" => {
                ...
                // Use static Regex (compiled once via lazy_static pattern)
                // For now we compile per-call; a future optimisation can use
                // `once_cell::sync::Lazy`.
                let re_percent = match Regex::new(r"(\d+\s?%)") {
                    Ok(r) => r,
                    Err(_) => {
                        eprintln!("InternalError: falha ao compilar regex para expressão matemática.");
                        return;
                    }
                };
                let re_percent_exp = match Regex::new(r"(\d+\s?%\s?\d+)") {
                    Ok(r) => r,
                    Err(_) => {
                        eprintln!("InternalError: falha ao compilar regex para expressão matemática.");
                        return;
