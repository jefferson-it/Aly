mod math {
    ...
    // to_sci(value) or to_exp(value) -> string representation in scientific notation
    pub fn fun_to_sci(x: String) -> Box<dyn Validator> {
        let args = parse_args(x);
        if args.is_empty() {
            return Box::new(put_quoted_str("0e0".to_string()));
        }
        let formatted = format!("{:e}", args[0]);
        Box::new(put_quoted_str(formatted))
    }

    pub fn fun_to_exp(x: String) -> Box<dyn Validator> {
        fun_to_sci(x)
    }

    // from_scientific(str) -> float/int
    pub fn fun_from_scientific(x: String) -> Box<dyn Validator> {
        let cleaned = x.trim().trim_matches('"').trim_matches('\'');
        if let Ok(val) = cleaned.parse::<f64>() {
            to_value(val)
        } else {
            Box::new(ValueData::Float(0.0))
        }
    }

    // bigint(str_or_number) -> string representation of arbitrary large integer
    pub fn fun_bigint(x: String) -> Box<dyn Validator> {
        let cleaned = x.trim().trim_matches('"').trim_matches('\'');
        Box::new(put_quoted_str(cleaned.to_string()))
    }
}
