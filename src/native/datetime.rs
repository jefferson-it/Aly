...
make_func!(datetime_timezone, |parts: Vec<String>| -> Box<dyn Validator> {
    let offset_hours: i32 = arg(&parts, 0).parse().unwrap_or(0);
    let offset_seconds = offset_hours * 3600;
    if let Some(offset) = FixedOffset::east_opt(offset_seconds) {
        let now = Utc::now().with_timezone(&offset);
        Box::new(ValueData::DateTime(now))
    } else {
        Box::new(ValueData::String("Invalid timezone offset".to_string()))
    }
});
