# Bindings Nativos & Extensibilidade no Aly

O Aly suporta extensibilidade em tempo de execução e bindings nativos de módulos Rust através de traits de registo nativos e bibliotecas dinâmicas.

---

## 1. Integração com Rust

Para vincular métodos Rust como funções nativas dentro do Aly, mapeia-se os tipos Rust utilizando a representação `ValueData` em módulos Rust (como `src/native/`):
...
Os plugins de bibliotecas externas (`.so` / `.dylib` / `.dll`) podem ser carregados dinamicamente no interpretador utilizando:

```aly
plugin import "caminho/para/plugin"
```

Isto regista símbolos, métodos e estruturas externos no namespace ativo de execução do Aly.

---

## 3. Importar um plugin
...
O caminho é relativo ao diretório atual de execução. O `as nome` define o namespace utilizado para chamar as funções do plugin. Se for omitido, o nome do ficheiro (sem extensão) é utilizado como namespace.

---

## 4. Chamar funções

As funções expostas pelo plugin são acedidas via `namespace.funcao(args)`:
...
Os argumentos são passados separados por vírgula ou espaço.
...
Qualquer plugin deve exportar três funções obrigatórias descritas no cabeçalho `plugin_sdk/plugin.h`:
...
Retorna um ponteiro para uma struct `AlyPluginInfo` estática. É chamada uma única vez ao carregar o plugin. Nunca deve retornar NULL.
...
static AlyPluginInfo info = {
    ...
    .name        = "Math Extension",
    .version     = "1.0.0",
    .description = "Funções matemáticas extra",
};
...
Recebe o nome da função e uma string de argumentos. Deve retornar uma string alocada com `malloc` (ou equivalente) que será libertada via `aly_plugin_free_string`. Retorne NULL para indicar um erro.
...
Liberta a string retornada por `aly_plugin_call`. Se o ponteiro for NULL, a chamada é ignorada.
...
Retorna uma string com os nomes das funções separados por vírgula. Se for exportada, cada função é registada individualmente como `namespace.nomefuncao`, permitindo chamadas diretas como `math.add(3, 7)`. Se não for exportada, o runtime regista apenas o namespace sem funções individuais.
...
static AlyPluginInfo info = {
    ...
    .name        = "Math Extension",
    .version     = "1.0.0",
    .description = "Funções matemáticas extra para o Aly",
};
...
Implemente o ABI utilizando `extern "C"` e `#[no_mangle]`:
...
static INFO: AlyPluginInfo = AlyPluginInfo {
    ...
    description: c"Funções SHA-256 para o Aly".as_ptr(),
};
...
## 11. Como o Aly carrega os plugins

Internamente, o Aly utiliza a biblioteca `libloading` (Rust) para carregar a biblioteca compartilhada em tempo de execução via `dlopen`/`dlsym` (Linux), `LoadLibrary`/`GetProcAddress` (Windows) ou `dlopen`/`dlsym` (macOS).

O fluxo de execução é:

1. O interpretador encontra a instrução `plugin import "path.so" as nome`.
2. Chama `aly_plugin_init()` para obter metadados e verificar a versão da API.
3. Chama `aly_plugin_functions()` (se for exportada) para descobrir as funções registadas.
4. Regista cada função como `nome.funcao` no runtime do Aly.
5. Quando `nome.funcao(args)` é executada, o Aly converte os argumentos para string e chama `aly_plugin_call("funcao", "arg1,arg2,...")`.
6. O resultado string é retornado e convertido de volta para o tipo adequado no Aly.

---

## 12. Segurança

Os plugins são código nativo executado no mesmo processo do Aly. Eles têm acesso total ao sistema (ficheiros, rede, subprocessos, etc.). Importe apenas plugins provenientes de fontes confiáveis.
