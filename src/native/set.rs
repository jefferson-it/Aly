mod set {
    ...
    pub struct Set(Vec<ValueData>);

    impl Set {
        pub fn new(value: Vec<ValueData>) -> Set {
            let mut final_data = vec![];

            for item in value {
                let (_, value_final) = item.valid();
                // Simple uniqueness check
                if !final_data.contains(&value_final) {
        ...
        pub fn to_string(&self, json: bool) -> String {
            let mut string = String::from("Set (#data");
    ...
    pub fn create_set(lexer: Vec<Lexer>) -> Box<dyn Validator>{
        let mut another = 0;
        let mut new_vec = vec![];
        let mut values = vec![];
        ...
        return Box::new(ValueData::Set(set));
    }
}
