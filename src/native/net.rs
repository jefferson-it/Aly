mod net_mod {
    ...
    // net.tls_send(host, port, data) -> TLS client request
    pub fn net_tls_send(x: String) -> Box<dyn Validator> {
        let args = split_args(&x, 3);
        let host = std_arg(&args, 0);
        let port = std_arg(&args, 1);
        let data = std_arg(&args, 2);

        // HTTPS/TLS connection via reqwest/rustls client
        let client = match reqwest::blocking::Client::builder().build() {
            Ok(c) => c,
            Err(e) => {
                eprintln!("RuntimeError [net.tls_send]: error creating client: {}", e);
                return Box::new(put_quoted_str(String::new()));
            }
        };

        let url = format!("https://{}:{}/", host, port);
        let res = match client.post(&url).body(data).send() {
            Ok(r) => r.text().unwrap_or_default(),
            Err(e) => {
                eprintln!("RuntimeError [net.tls_send]: request error: {}", e);
                String::new()
            }
        };

        Box::new(put_quoted_str(res))
    }
}
