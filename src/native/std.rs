...
// fs.mov(src, dst) -> bool (success). Renames/moves a file.
pub fn fs_mov(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 2);
    let src = arg(&args, 0);
    let dst = arg(&args, 1);
    ok_bool(fs::rename(&src, &dst).is_ok())
}
...
// fs.copy(src, dst) -> bool (success)
pub fn fs_copy(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 2);
    let src = arg(&args, 0);
    let dst = arg(&args, 1);
    ok_bool(fs::copy(&src, &dst).is_ok())
}
...
// sys.hostname() -> hostname string
pub fn sys_hostname(_x: String) -> Box<dyn Validator> {
...
// fs.home() -> home directory path
pub fn fs_home(_x: String) -> Box<dyn Validator> {
...
pub fn fs_is_file(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 1);
    let path = arg(&args, 0);

    ok_bool(Path::new(&path).is_file())
}

// fs.touch(path) -> bool (creates empty file or updates timestamp)
pub fn fs_touch(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 1);
    let path = arg(&args, 0);
    let p = Path::new(&path);

    if !p.exists() {
        return ok_bool(fs::write(&path, "").is_ok());
    }

    match fs::read(&path) {
        Ok(content) => ok_bool(fs::write(&path, content).is_ok()),
        Err(_) => ok_bool(fs::write(&path, "").is_ok()),
    }
}
...
// str.join(sep, part1, part2, ...) -> joined string
pub fn str_join(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 0);
    if args.len() < 2 {
        return ok_str(String::new());
    }
    let sep = arg(&args, 0);
    let parts: Vec<String> = (1..args.len()).map(|i| arg(&args, i)).collect();
    ok_str(parts.join(&sep))
}

// vec_join(vec, sep) -> joined string (vector elements joined by sep)
// Expects a JSON-like vector string, e.g. "[1, 2, 3]"
pub fn vec_join(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 2);
    let vec_str = arg(&args, 0);
    let sep = arg(&args, 1);

    let inner = vec_str.trim();
...
// str.reverse(s) -> reversed string
pub fn str_reverse(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 1);
    ok_str(arg(&args, 0).chars().rev().collect())
}

// str.pad(s, width, fill) -> padded string
pub fn str_pad(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 3);
    let s = arg(&args, 0);
    let width: usize = arg(&args, 1).trim().parse().unwrap_or(0);
    let fill = arg(&args, 2);
    if s.len() >= width { return ok_str(s); }
    let pad_len = width - s.len();
    let mut result = s.clone();
    for _ in 0..pad_len { result.push_str(&fill); }
    ok_str(result)
}

// str.lstrip(s, chars) -> left-stripped string
pub fn str_lstrip(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 2);
    let s = arg(&args, 0);
    let chars = arg(&args, 1);
    ok_str(s.trim_start_matches(&chars).to_string())
}

// str.rstrip(s, chars) -> right-stripped string
pub fn str_rstrip(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 2);
    let s = arg(&args, 0);
    let chars = arg(&args, 1);
    ok_str(s.trim_end_matches(&chars).to_string())
}

// math.abs(n) -> absolute value
pub fn math_abs(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 1);
    let n: f32 = arg(&args, 0).trim().parse().unwrap_or(0.0);
    ok_int(n.abs() as i32)
}

// math.max(a, b) -> greater of two numbers
pub fn math_max(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 2);
    let a: f32 = arg(&args, 0).trim().parse().unwrap_or(0.0);
    let b: f32 = arg(&args, 1).trim().parse().unwrap_or(0.0);
    ok_int(a.max(b) as i32)
}

// math.min(a, b) -> smaller of two numbers
pub fn math_min(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 2);
    let a: f32 = arg(&args, 0).trim().parse().unwrap_or(0.0);
    let b: f32 = arg(&args, 1).trim().parse().unwrap_or(0.0);
    ok_int(a.min(b) as i32)
}

// math.round(n) -> nearest integer
pub fn math_round(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 1);
    let n: f32 = arg(&args, 0).trim().parse().unwrap_or(0.0);
    ok_int(n.round() as i32)
}

// math.floor(n) -> largest integer <= n
pub fn math_floor(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 1);
    let n: f32 = arg(&args, 0).trim().parse().unwrap_or(0.0);
    ok_int(n.floor() as i32)
}

// math.ceil(n) -> smallest integer >= n
pub fn math_ceil(x: String) -> Box<dyn Validator> {
    let args = split_args(&x, 1);
    let n: f32 = arg(&args, 0).trim().parse().unwrap_or(0.0);
    ok_int(n.ceil() as i32)
}
