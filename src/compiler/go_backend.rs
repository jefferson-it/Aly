...
pub fn compile_to_go(source: &str) -> Result<String, String> {
    ...
    go_code.push_str("// Generated from Aly source\n");
    go_code.push_str("package main\n\n");
    go_code.push_str("import \"fmt\"\n\n");
    
    // Main function body
    go_code.push_str("func main() {\n");
    
    let transpiler = GoTranspiler;
    for stmt in &program.stmts {
        let stmt_code = transpiler.transpile_stmt(stmt, 1)?;
        go_code.push_str(&stmt_code);
    }
...
impl GoTranspiler {
    fn transpile_stmt(&self, stmt: &Stmt, indent: usize) -> Result<String, String> {
        ...
        match stmt {
            ...
            Stmt::Fun { name, params, body, .. } => {
                let mut code = format!("{}var {} = func({}) {{\n", ind, name, params.join(", interface{}"));
                if !params.is_empty() {
                    code = code.replace("func(", "func(").replace(" {", " interface{} {");
                }
                for s in body {
                    code.push_str(&self.transpile_stmt(s, indent + 1)?);
                }
            ...
            _ => Ok(format!("{}// stmt not supported in Go transpile\n", ind)),
        }
    }
    ...
    fn transpile_expr(&self, expr: &Expr) -> Result<String, String> {
        match expr {
            ...
            Expr::None => Ok("nil".to_string()),
            Expr::Void => Ok("struct{}{}".to_string()),
            Expr::Var(name) => Ok(name.clone()),
            Expr::TemplateStr(parts) => {
                let mut go_format = String::new();
                let mut go_args = Vec::new();
                for part in parts {
                    match part {
                        TemplatePart::Text(t) => go_format.push_str(&t.replace("%", "%%")),
                        TemplatePart::Var(v) => {
                            go_format.push_str("%v");
                            go_args.push(v.clone());
                        }
                    }
                }
                if go_args.is_empty() {
                    ...
                    Ok(format!("fmt.Sprintf(\"{}\", {})", go_format, go_args.join(", ")))
                }
            }
            ...
            Expr::BinOp { left, op, right } => {
                ...
                Ok(format!("({} {} {})", left_code, go_op, right_code))
            }
            Expr::Call { function, args } => {
                ...
                if let Expr::Var(name) = &**function {
                    if name == "print" {
                        return Ok(format!("fmt.Println({})", arg_codes.join(", ")));
                    }
                }
